...
import { GoogleAuthProvider, onAuthStateChanged, sendEmailVerification } from "firebase/auth";
    ...
    const handleSubmit=(event)=>{
       ...
       const passwordRegex = /^(?=.*[@#$%^&+=])(?=\S+).{4,}$/
        if (!passwordRegex.test(password)) {
            setError('Password should be at least one lowercase letter,atleast one uppercase letter,atleast one digit and minimum length of 8 characters');
            return;
        }

        //confrim password

        const confirmpasswordRegex = /^(?=.*[@#$%^&+=])(?=\S+).{4,}$/
        ...
        setError('User name should be  6 characters that can be either uppercase letters, lowercase letters, or digits');
        return;
         }
        //email regex
...
if (!bangladeshiPhoneNumberRegex.test(phone)) {
  ...
  <input type="tel" className="form-control" id="phone" placeholder="Enter your phone number" />
    ...
    <Row>
      ...
      <p className='text-info'>{sucess}</p>
      </div>
      </Col>
    </Row>
        </Col>
