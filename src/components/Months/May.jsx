import { Col, Row } from "react-bootstrap";
import { Link } from "react-router-dom";


const May = () => {
    return (
        <div>
        <p style={{fontFamily:"cursive",marginTop:'50px',marginLeft:'160px',fontSize:'52px'}} className="text-center text-danger "></p>
        <Link style={{fontSize:'30px',marginLeft:"1800px"}} to="/season">Back to</Link>
        <Row>
            
            <Col style={{backgroundColor:'violet',height:'500px',width:'500px'}}className="col-md-4 px-5 mx-5 mt-5">
            <p className="fs-4 text-white pt-5"> In Bangladesh, May marks the beginning of the hot and humid pre-monsoon season. During this time, various fruits continue to be in season. Here are some fruits that are commonly available in Bangladesh during May:</p>
            </Col>
            <Col className="mt-5 pt-5">
                <Row>
                    <Col> <img className="w-75 pe-4 ms-5" src="../../images/ortt.jpg"/>
                    <h2 className="text-center text-danger">Jamrul </h2>
                    </Col>
                    <Col> <img className="w-75 pe-4" src="../../images/or4.jpg"/>
                    <h2 className="text-center text-danger mt-5 pt-5">Banana</h2>
                    </Col>
                    <Col> <img className="w-100 pe-4" src="../../images/or2.jpg"/>
                    <h2 className="text-center text-danger">Guava</h2>
                    </Col>
                    
                    
          
           

                </Row>
                <Row className="mt-5">
                <Col> <img className="w-100 pe-4" src="../../images/or11.jpg"/>
                    <h2 className="text-center text-danger pt-5 mt-2">Orange</h2>
                    </Col>
                    <Col> <img className="w-100 pe-4 pt-3" src="../../images/or6.jpg"/>
                    <h2 className="text-center text-danger pt-5 mt-4">Lichi</h2>
                    </Col>
                    <Col> <img className="w-100 pe-4" src="../../images/or7.jpg"/>
                    <h2 className="text-center text-danger">Pineapple</h2>
                    </Col>
             
             
                    <Col> <img className="w-75 pe-4" src="../../images/or77.png"/>
                    <h2 className="text-center text-danger pt-5 mt-4">Jambura</h2>
                    </Col>
          
     

                </Row>
         
         
           
            </Col>


            
            
        </Row>
       </div>
    );
};

export default May;